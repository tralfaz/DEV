...
class SliceFormatter(string.Formatter):
    ...
    def get_value(self, key, args, kwds):
        print "GET VAL: KEY:%s ARGS:%s KWDS:%s" % (repr(key), repr(args), repr(kwds))
        if '|' in key:
            try:
                key, indexes = key.split('|')
                indexes = [int(idx) if idx else None
...
class ElideFormatter(string.Formatter):

    def get_value(self, key, args, kwds):
        print "GET VAL: KEY:%s ARGS:%s KWDS:%s" % (repr(key), repr(args), repr(kwds))
        if '$' in key:
            try:
                key, elidespec = key.split('$')
                elidespec = urllib.unquote(elidespec)
                value = args[int(key)] if key.isdigit() else kwds[key]
                if '<' in elidespec:
                    return self.pre_elide_value(elidespec, value)
                elif '>' in elidespec:
                    return self.post_elide_value(elidespec, value)
                elif '^' in elidespec:
                    return self.center_elide_value(elidespec, value)
                else:
                    raise ValueError('invalid eliding option %r' % elidespec)
            except KeyError:
                return kwds.get(key, 'Missing')
        return super(ElideFormatter,self).get_value(key, args, kwds)

    def center_elide_value(self, elidespec, value):
    ...
    def post_elide_value(self, elidespec, value):
        elidelen, elidetxt = elidespec.split('>')
        elen = int(elidelen)
        vlen = len(value)
        tlen = len(elidetxt)
    ...
    def pre_elide_value(self, elidespec, value):
        elidelen, elidetxt = elidespec.split('<')
        elen = int(elidelen)
        vlen = len(value)
        tlen = len(elidetxt)
...
#  pre $10<...  post $10>... cent $10^...

if __name__ == '__main__':
  phrase = "Hello |{name|5,:>8}|, nice to meet you.  I am {name|6,9}.  That is {0|0,4}."
  fmt = SliceElideFormatter()
  print fmt.format(phrase, "JeffJeffJeff", name="Larry Bob")


  print
  efmtr = ElideFormatter()
  edata = { 'PRE':'prefixedtextvalue' }
  prefmt = '{PRE$10<%2E%2E%2E:10.10}'
  print 'PREFMT<: 1234567890\n        %r' % efmtr.format(prefmt, *(), **edata)
  prefmt = '{PRE$17<%2E%2E%2E}'
  print 'PREFMT=: 12345678901234567\n        %r' % efmtr.format(prefmt, *(), **edata)
  prefmt = '{PRE$19<%2E%2E%2E:>19}'
  print 'PREFMT<: 1234567890123456789\n        %r' % efmtr.format(prefmt, *(), **edata)

  print
  edata = { 'POST':'postfixedtextvalue' }
  prefmt = '{POST$10>%2E%2E%2E:10.10}'
  print 'POSTFMT<: 1234567890\n         %r' % efmtr.format(prefmt, *(), **edata)
  prefmt = '{POST$18>%2E%2E%2E}'
  print 'POSTFMT=: 123456789012345678\n         %r' % efmtr.format(prefmt, *(), **edata)
  prefmt = '{POST$19>%2E%2E%2E:>19}'
  print 'POSTFMT<: 1234567890123456789\n         %r' % efmtr.format(prefmt, *(), **edata)

  print
  edata = { 'CNT':'centeredtextvalue' }
  fmt = '{CNT$11^%2E%2E%2E:11.11}'
  print 'CNTFMT<: 1234567890\n        %r' % efmtr.format(fmt, *(), **edata)
  edata = { 'CNT':'centeredtextvalueX' }
  print 'CNTFMT<: 1234567890\n        %r' % efmtr.format(fmt, *(), **edata)
#  fmt = '{PRE$17<%2E%2E%2E}'
#  print 'PREFMT=: 12345678901234567\n        %r' % efmtr.format(prefmt, *(), **edata)
#  prefmt = '{PRE$19<%2E%2E%2E:>19}'
